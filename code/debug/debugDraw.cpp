...
namespace
{
    ...
    {
        int width = Fishing::SCREEN_WIDTH;
        int height = Fishing::SCREEN_HEIGHT;
...
void Debug::draw(uint16_t *fb)
{
    ...
    float maxX = Fishing::SCREEN_WIDTH;
    float maxY = Fishing::SCREEN_HEIGHT;
    for (auto &line : lines)
    {
        ...
        if (line.a.y < 0 && line.b.y < 0)
            continue;
        if (line.a.x > maxX && line.b.x > maxX)
            continue;
        if (line.a.y > maxY && line.b.y > maxY)
            continue;
